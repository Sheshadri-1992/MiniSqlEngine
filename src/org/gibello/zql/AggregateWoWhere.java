...
public class AggregateWoWhere {
    ...
    AggregateWoWhere(Vector arg_vec,HashMap<String,Integer> att_pos) throws FileNotFoundException
    {
		v=arg_vec;
		my_att_pos=att_pos;
		hold_columns = new Vector<Vector<Integer>>();
//		reader_x = new FileReader("Output.txt");
...
//		System.out.println(my_att_pos.toString());
	...
	public void agg_function() throws IOException
	{
		...
		int row=0;
//		System.out.println("Number of lines in Output.txt are " + count);
		int minimum;
		while(line!=null)
		{
...
//				System.out.println("control should reach here");
				minimum = my_array[0][position];
//				System.out.println("Min is "+my_array[0][position]);

				for(int k=0;k<count;k++)
				{
//					System.out.println("Min is "+my_array[k][position]);

					if(minimum>my_array[k][position])
					{
						minimum=my_array[k][position];
//						System.out.println("Min is inside for loop "+my_array[k][position]);
					}
				}
